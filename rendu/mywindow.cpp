...
void myWindow::paintGL()
{
    ...
    if(!meshUpToDate){
        ...
        for(float i=-largeur/2;i<largeur/2;i+=2){
            for(float j=-longueur/2; j<longueur/2; j+=2){
                int tmp = (int)rand()%4;
                tmp = 0;
                //qDebug()<<tmp;
                if(tmp == 0){
                    //PaterneQuadResidence p1(Vector2D(i,j), Vector2D(i,j+3), Vector2D(i+3,j+3), Vector2D(i+3,j),&_par);
