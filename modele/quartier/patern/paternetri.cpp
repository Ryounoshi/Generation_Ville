...
void PaterneTri::processTrapezoidObtuseStart(const Vector2D& p1, const Vector2D& p2, const Vector2D& p3, const Vector2D& p4)
{
	/*
	*   p1       p4
	*    p2       p3
	*/
	...
	float triEndLength = (p4 - p3).scalareProduct(p2 - p3) / (p2 - p3).getNorm() + EPSILON; // longueur du projeté de p4 sur p3-p2
	Vector2D dir_processedSide = Normalized(p3 - p2);
	Vector2D dir_height = Vector2D(dir_processedSide.y, -dir_processedSide.x);
	...
	do {
		...
		if (bat_length < MIN_DIM_BAT) {
			waterMark = waterMark + bat_length*dir_processedSide;
			continue;
		}
		Vector2D wmark_candBat = waterMark + bat_length*dir_processedSide;
		if (waterMark == p2)
		{
			...
			if ((wmark_candBat - p2).getNorm() < (p3 - p2).getNorm() - triEndLength) {
				saveBatTrapezoid(waterMark, wmark_candBat, hp, hp, dir_height);
				waterMark = wmark_candBat;
				continue;
			}
			else {
...
void PaterneTri::processTrapezoidObtuseEnd(const Vector2D& p1, const Vector2D& p2, const Vector2D& p3, const Vector2D& p4)
{
	/*
	*     p1       p4
	*    p2       p3
	*/
	...
	float hp = (to3D(p3 - p2) ^ to3D(p1 - p2)).getNorm() / (p3 - p2).getNorm();
	float triStartLength = (p1 - p2).scalareProduct(p3 - p2) / (p3 - p2).getNorm() + EPSILON; // longueur du projeté de p1 sur p2-p3
	Vector2D dir_processedSide = Normalized(p3 - p2);
	Vector2D dir_height = Vector2D(dir_processedSide.y, -dir_processedSide.x);
	Vector2D waterMark = p2; // sera p2 ou après triangle
	auto distribution = make_Unidistrib(0, (p3 - waterMark).getNorm());
	do {
		...
		if (waterMark == p2 && bat_length <= triStartLength) { // on veut que des trapezes
			waterMark = p2 + triStartLength*dir_processedSide;
			continue;
		}
		if (bat_length > (p3 - waterMark).getNorm() || bat_length > MAX_DIM_BAT) {
			bat_length = std::min((p3 - waterMark).getNorm(), MAX_DIM_BAT);
...
void PaterneTri::processTrapezoid(const Vector2D& p1, const Vector2D& p2, const Vector2D& p3, const Vector2D& p4)
{
	if ((p1 - p2).scalareProduct(p3 - p2) < 0) {
		processTrapezoidObtuseStart(p1, p2, p3, p4);
		return;
	}
	if ((p4 - p3).scalareProduct(p2 - p3) < 0) {
		processTrapezoidObtuseEnd(p1, p2, p3, p4);
		return;
	}
...
/**
 * Creates a trapezoid Batiment.
 *
 * @param pt1
 * @param pt2
 * @param depthPt1 the length from pt1
 * @param depthPt2 the length from pt2
 * @param dir_height the direction where the new points will be
 */
void PaterneTri::saveBatTrapezoid(const Vector2D& pt1, const Vector2D& pt2, float depthPt1, float depthPt2, const Vector2D& dir_height)
{
...
/**
 * Adds buildings to the sides of the triangle.
 */
void PaterneTri::split()
{
	...
	*/
	...
	if (orientation(pts[0], pts[1], pts[2]) < 0)
		tronq = processSide(pts[0], pts[1], pts[2]);
	else
		tronq = processSide(pts[1], pts[0], pts[2]);
	...
	if (orientation(tronq.second, pts[2], tmp) < 0)
		tronq = processSide(tronq.second, pts[2], tmp);
	else
		tronq = processSide(pts[2], tronq.second, tmp);
	...
	if (orientation(tronq.second, tmp, tronq.first) < 0)
		processSide(tronq.second, tmp, tronq.first);
	else
		processSide(tmp, tronq.second, tronq.first);
}

/**
 * Makes a trapezoid with the side formed by the first 2 parameters.
 *
 * @return the cut points that replace the first 2 points to make another triangle
 */
std::pair<Vector2D, Vector2D> PaterneTri::processSide(const Vector2D& pp2, const Vector2D& pp1, const Vector2D& pp3)
{
	/*
		 	 pp3
			   |
	...
	*/
	...
	Vector2D dir_processedSide = Normalized(pp1 - pp2);
	Vector2D dir_height(dir_processedSide.y, -dir_processedSide.x);
	if ((pp3 - pp1).scalareProduct(dir_height) > 0) // orientation
		processTrapezoid(lambda, pp2, pp1, mu);
	else
		processTrapezoid(mu, pp1, pp2, lambda);
